...
{
    public partial class SettingVisitTo : Form
    {
        public SettingVisitTo()
        {
            InitializeComponent();
        }

        int lstSelectedIndex = -1;
        private void SettingVisitTo_Load(object sender, EventArgs e)
        {
        ...
        private void GET_LISTS()
        {
            ...
            {
                OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id, visit_to FROM tbl_visit_to ORDER BY id ASC", classGlobal.conn);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            ...
            {
                NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id, visit_to FROM tbl_visit_to ORDER BY id ASC", classGlobal.connP);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            ...
            {
                dt.Columns.Add("id");
                dt.Columns.Add("visit_to");

                string jsonString = ClassData.GET_METHODE("visitPerson");
                if (jsonString == "")
                {
                    //-- no data
                }
            ...
            if (ret == "YES")
            {
                try
                {
                    ...
                    if (classGlobal.databaseType == "acc")
                    {
                        ...
                        command.Parameters.Add("@visit_to", OleDbType.VarChar).Value = textBox2.Text;
                    ...
                    else if (classGlobal.databaseType == "psql")
                    {
                        ...
                        command.Parameters.AddWithValue("@visit_to",  textBox2.Text);
                        command.ExecuteNonQuery();
                        command.Dispose();
                        command = null;
                    }
                    ...
                    {
                        string s = ClassData.POST_METHODE("visitPerson", textBox2.Text);
                        if (s != "200")
                        {
                            frmMessageBox f = new frmMessageBox();
                            f.strMessage = s;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessageBox f = new frmMessageBox();
                f.strMessage = "กรุณาเลือกผู้รับการติดต่อ";
                f.strStatus = "Warning";
                f.ShowDialog();
                return;
            }
            ...
            if (ret == "YES")
            {
                ...
                string query = "DELETE FROM tbl_visit_to WHERE id=" + lstSelectedIndex;
                if (classGlobal.databaseType == "acc")
                {
                    OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                    command.ExecuteNonQuery();
                ...
                {
                    string s = ClassData.DELETE_METHODE("visitPerson", textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
        ...
        private int GET_ID(string str)
        {
            ...
            try
            {
                ...
                if (classGlobal.databaseType == "acc")
                {
                    OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id FROM tbl_visit_to WHERE visit_to ='" + str + "'", classGlobal.conn);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                ...
                {
                    NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id FROM tbl_visit_to WHERE visit_to ='" + str + "'", classGlobal.connP);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                    ret = Int32.Parse(dt.Rows[0].ItemArray[0].ToString());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (ret == "YES")
            {
                ...
                string query = "UPDATE tbl_visit_to SET visit_to = '" + textBox2.Text + "' WHERE id=" + lstSelectedIndex;
                if (classGlobal.databaseType == "acc")
                {
                    OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                    command.ExecuteNonQuery();
                ...
                {
                    string s = ClassData.PUT_METHODE("visitPerson", listBox1.SelectedItem.ToString(), textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            {
                                OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                                command.Parameters.Add("@visit_to", OleDbType.VarChar).Value = dr[0].ToString();
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
                            ...
                            {
                                NpgsqlCommand command = new NpgsqlCommand(query, classGlobal.connP);
                                command.Parameters.AddWithValue("@visit_to", dr[0].ToString());
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
