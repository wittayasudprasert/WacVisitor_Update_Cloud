...
{
    public partial class SettingPlace : Form
    {
        public SettingPlace()
        {
            InitializeComponent();
        ...
        private void SettingPlace_Load(object sender, EventArgs e)
        {
        ...
        private void GET_LISTS()
        {
            ...
            {
                OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id, place FROM tbl_place ORDER BY id ASC", classGlobal.conn);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            ...
            {
                NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id, place FROM tbl_place ORDER BY id ASC", classGlobal.connP);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            else
            {
                ...
                string jsonString = ClassData.GET_METHODE("visitPlace");
                if (jsonString == "")
                {
                    //-- no data
                }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            classGlobal.MsgText = "ต้องการเพิ่มสถานที่ติดต่อ?";
            Msg m = new Msg();
            m.ShowDialog();
            ...
            if (ret == "YES")
            {
                try
                {
                    ...
                    if (classGlobal.databaseType == "acc")
                    {
                        ...
                        command.Parameters.Add("@place", OleDbType.VarChar).Value = textBox2.Text;
                    ...
                    {
                        string s = ClassData.POST_METHODE("visitPlace", textBox2.Text);
                        if (s != "200")
                        {
                            frmMessageBox f = new frmMessageBox();
                            f.strMessage = s;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessageBox f = new frmMessageBox();
                f.strMessage = "กรุณาเลือกสถานที่ติดต่อ";
                f.strStatus = "Warning";
                f.ShowDialog();
                return;
            }
            ...
            if (ret == "YES")
            {
                ...
                string query = "DELETE FROM tbl_place WHERE id=" + lstSelectedIndex;
                if (classGlobal.databaseType == "acc")
                {
                    OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                    command.ExecuteNonQuery();
                ...
                {
                    string s = ClassData.DELETE_METHODE("visitPlace", textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
        ...
        private int GET_ID(string str)
        {
            ...
            try
            {
                ...
                if (classGlobal.databaseType == "acc")
                {
                    OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id FROM tbl_place WHERE place ='" + str + "'", classGlobal.conn);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                ...
                {
                    NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id FROM tbl_place WHERE place ='" + str + "'", classGlobal.connP);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                    ret = Int32.Parse(dt.Rows[0].ItemArray[0].ToString());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (ret == "YES")
            {
                ...
                string query = "UPDATE tbl_place SET place = '" + textBox2.Text + "' WHERE id=" + lstSelectedIndex;
                ...
                {
                    string s = ClassData.PUT_METHODE("visitPlace", listBox1.SelectedItem.ToString(), textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            {
                                OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                                command.Parameters.Add("@place", OleDbType.VarChar).Value = dr[0].ToString();
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
                            ...
                            {
                                NpgsqlCommand command = new NpgsqlCommand(query, classGlobal.connP);
                                command.Parameters.AddWithValue("@place", dr[0].ToString());
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
