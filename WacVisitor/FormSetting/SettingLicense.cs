...
{
    public partial class SettingLicense : Form
    {
        public SettingLicense()
        {
            InitializeComponent();
        }

        int lstSelectedIndex = -1;
        private void SettingLicense_Load(object sender, EventArgs e)
        {
        ...
        private void GET_LICENSE_LISTS()
        {
            DataTable dt = new DataTable("dt");

            if (classGlobal.databaseType == "acc")
            {
                OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id, license_plate FROM tbl_license_plate ORDER BY id ASC", classGlobal.conn);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            ...
            {
                NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id, license_plate FROM tbl_license_plate ORDER BY id ASC", classGlobal.connP);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            else
            {
                ...
                string jsonString = ClassData.GET_METHODE("licensePlate");
                if (jsonString == "")
                {
                    //-- no data
                }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            classGlobal.MsgText = "ต้องการเพิ่มเลขทะเบียนรถ?";
            Msg m = new Msg();
            m.ShowDialog();
            ...
            if (ret == "YES")
            {
                try
                {
                    ...
                    if (classGlobal.databaseType == "acc")
                    {
                        ...
                        command.Parameters.Add("@license_plate", OleDbType.VarChar).Value = textBox2.Text;
                    ...
                    else if (classGlobal.databaseType == "psql")
                    {
                        ...
                        command.Parameters.AddWithValue("@license_plate", textBox2.Text);
                        command.ExecuteNonQuery();
                        command.Dispose();
                        command = null;
                    }
                    ...
                    {
                        string s = ClassData.POST_METHODE("licensePlate", textBox2.Text);
                        if (s != "200")
                        {
                            frmMessageBox f = new frmMessageBox();
                            f.strMessage = s;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessageBox f = new frmMessageBox();
                f.strMessage = "กรุณาเลือกเลขทะเบียนรถ";
                f.strStatus = "Warning";
                f.ShowDialog();
                return;
            }
            ...
            if (ret == "YES")
            {
                ...
                string query = "DELETE FROM tbl_license_plate WHERE id=" + lstSelectedIndex;
                ...
                {
                    string s = ClassData.DELETE_METHODE("licensePlate", textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
        ...
        private int GET_ID(string str)
        {
            ...
            try
            {
                ...
                if (classGlobal.databaseType == "acc")
                {
                    OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id FROM tbl_license_plate WHERE license_plate ='" + str + "'", classGlobal.conn);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                ...
                {
                    NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id FROM tbl_license_plate WHERE license_plate ='" + str + "'", classGlobal.connP);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                    ret = Int32.Parse(dt.Rows[0].ItemArray[0].ToString());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (ret == "YES")
            {
                ...
                string query = "UPDATE tbl_license_plate SET license_plate = '" + textBox2.Text + "' WHERE id=" + lstSelectedIndex;
                ...
                {
                    string s = ClassData.PUT_METHODE("licensePlate", listBox1.SelectedItem.ToString(), textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            {
                                OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                                command.Parameters.Add("@license_plate", OleDbType.VarChar).Value = dr[0].ToString();
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
                            ...
                            {
                                NpgsqlCommand command = new NpgsqlCommand(query, classGlobal.connP);
                                command.Parameters.AddWithValue("@license_plate", dr[0].ToString());
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
