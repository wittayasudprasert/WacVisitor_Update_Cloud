...
{
    public partial class SettingBusinessTopic : Form
    {
        public SettingBusinessTopic()
        {
            InitializeComponent();
        }
        int lstSelectedIndex = -1;
        private void SettingBusinessTopic_Load(object sender, EventArgs e)
        {
            ...
            linkLabel1.Left = (panel1.Width - linkLabel1.ClientSize.Width) / 2;
        ...
        private void GET_LISTS()
        {
            ...
            {
                OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id, business_topic FROM tbl_business_topic ORDER BY id ASC", classGlobal.conn);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            ...
            {
                NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id, business_topic FROM tbl_business_topic ORDER BY id ASC", classGlobal.connP);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            ...
            {
                dt.Columns.Add("id");
                dt.Columns.Add("business_topic");

                string jsonString = ClassData.GET_METHODE("contactTopic");
                if (jsonString == "")
                {
                    //-- no data
                }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            classGlobal.MsgText = "ต้องการเพิ่มผู้รับการติดต่อ?";
            Msg m = new Msg();
            m.ShowDialog();
            ...
            if (ret == "YES")
            {
                try
                {
                    ...
                    if (classGlobal.databaseType == "acc")
                    {
                        ...
                        command.Parameters.Add("@business_topic", OleDbType.VarChar).Value = textBox2.Text;
                    ...
                    else if (classGlobal.databaseType == "psql")
                    {
                        ...
                        command.Parameters.AddWithValue("@business_topic",textBox2.Text);
                        command.ExecuteNonQuery();
                        command.Dispose();
                        command = null;
                    }
                    ...
                    {
                        string s = ClassData.POST_METHODE("contactTopic", textBox2.Text);
                        if (s != "200")
                        {
                            frmMessageBox f = new frmMessageBox();
                            f.strMessage = s;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessageBox f = new frmMessageBox();
                f.strMessage = "กรุณาเลือกติดต่อเรื่อง";
                f.strStatus = "Warning";
                f.ShowDialog();
                return;
            }
            ...
            if (ret == "YES")
            {
                ...
                string query = "DELETE FROM tbl_business_topic WHERE id=" + lstSelectedIndex;                
                if (classGlobal.databaseType == "acc")
                {
                    OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                    command.ExecuteNonQuery();
                ...
                {
                    string s = ClassData.DELETE_METHODE("contactTopic", textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
        ...
        private int GET_ID(string str)
        {
            ...
            try
            {
                ...
                if (classGlobal.databaseType == "acc")
                {
                    OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id FROM tbl_business_topic WHERE business_topic ='" + str + "'", classGlobal.conn);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                ...
                {
                    NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id FROM tbl_business_topic WHERE business_topic ='" + str + "'", classGlobal.connP);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                    ret = Int32.Parse(dt.Rows[0].ItemArray[0].ToString());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (ret == "YES")
            {
                ...
                string query = "UPDATE tbl_business_topic SET business_topic = '" + textBox2.Text + "' WHERE id=" + lstSelectedIndex;
                if (classGlobal.databaseType == "acc")
                {
                    OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                    command.ExecuteNonQuery();
                ...
                {
                    string s = ClassData.PUT_METHODE("contactTopic", listBox1.SelectedItem.ToString(), textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            {
                                OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                                command.Parameters.Add("@business_topic", OleDbType.VarChar).Value = dr[0].ToString();
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
                            ...
                            {
                                NpgsqlCommand command = new NpgsqlCommand(query, classGlobal.connP);
                                command.Parameters.AddWithValue("@business_topic", dr[0].ToString());
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
