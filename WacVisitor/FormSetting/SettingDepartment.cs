...
namespace WacVisitor.FormSetting
{
    public partial class SettingDepartment : Form
    {
        ...
        private void GET_LISTS()
        {
            ...
            {
                OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id, department FROM tbl_department ORDER BY id ASC", classGlobal.conn);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            ...
            {
                NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id, department FROM tbl_department ORDER BY id ASC", classGlobal.connP);
                adapter.Fill(dt);
                adapter.Dispose();
                adapter = null;
            }
            else
            {
                ...
                string jsonString = ClassData.GET_METHODE("department");
                if (jsonString == "")
                {
                    //-- no data
                }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (ret == "YES")
            {
                try
                {
                    string query = "INSERT INTO tbl_department (department) VALUES (@department)";
                    if (classGlobal.databaseType == "acc")
                    {
                        OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                        command.Parameters.Add("@department", OleDbType.VarChar).Value = textBox2.Text;
                    ...
                    {
                        string s = ClassData.POST_METHODE("department", textBox2.Text);
                        if (s != "200")
                        {
                            frmMessageBox f = new frmMessageBox();
                            f.strMessage = s;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessageBox f = new frmMessageBox();
                f.strMessage = "กรุณาเลือกแผนกที่ติดต่อ";
                f.strStatus = "Warning";
                f.ShowDialog();
                return;
            }
            ...
            if (ret == "YES")
            {
                ...
                string query = "DELETE FROM tbl_department WHERE id=" + lstSelectedIndex;
                ...
                {
                    string s = ClassData.DELETE_METHODE("department", textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
        ...
        private int GET_ID(string str)
        {
            ...
            try
            {
                ...
                if (classGlobal.databaseType == "acc")
                {
                    OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT id FROM tbl_department WHERE department ='" + str + "'", classGlobal.conn);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                ...
                {
                    NpgsqlDataAdapter adapter = new NpgsqlDataAdapter("SELECT id FROM tbl_department WHERE department ='" + str + "'", classGlobal.connP);
                    adapter.Fill(dt);
                    adapter.Dispose();
                    adapter = null;
                    ret = Int32.Parse(dt.Rows[0].ItemArray[0].ToString());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (ret == "YES")
            {
                ...
                string query = "UPDATE tbl_department SET department = '" + textBox2.Text + "' WHERE id=" + lstSelectedIndex;
                ...
                {
                    string s = ClassData.PUT_METHODE("department", listBox1.SelectedItem.ToString(), textBox2.Text);
                    if (s != "200")
                    {
                        frmMessageBox f = new frmMessageBox();
                        f.strMessage = s;
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            {
                                OleDbCommand command = new OleDbCommand(query, classGlobal.conn);
                                command.Parameters.Add("@department", OleDbType.VarChar).Value = dr[0].ToString();
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
                            ...
                            {
                                NpgsqlCommand command = new NpgsqlCommand(query, classGlobal.connP);
                                command.Parameters.AddWithValue("@department", dr[0].ToString());
                                command.ExecuteNonQuery();
                                command.Dispose();
                                command = null;
                            }
